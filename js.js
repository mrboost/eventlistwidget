...
let getBadge = months => {
    ...
    return `<img src="${badges[badge].image_url_2x}" class="badge"/>`;
};
...
let parseEvent = event => {
    ...
    if (event.type === 'follower') {
        ...
        if (event.amount === 'gift') {
            ...
            addEvent('sub', `sub`, `${prefix} ${event.name}`);
        }
...
function addEvent(type, text, username) {
    totalEvents += 1;
    let element = `<li class="event-container" id="event-${totalEvents}"><div>
    ...
    if (direction === "bottom") {
        $('.widget-EventList').removeClass("fadeOutClass").show().prepend(element);
    } else {
        $('.widget-EventList').removeClass("fadeOutClass").show().append(element);
    }
    if (fadeoutTime !== 999) {
        $('.widget-EventList').addClass("fadeOutClass");
    }
